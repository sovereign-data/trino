        ...
        implements PasswordAuthenticatorFactory
{
    ...
    public PasswordAuthenticator create(Map<String, String> config)
    {
        Bootstrap app = new Bootstrap(
                binder -> {
                    configBinder(binder).bindConfig(KeycloakAuthenticatorConfig.class);
                    binder.bind(KeycloakAuthenticatorClient.class).in(Scopes.SINGLETON);
                    binder.bind(KeycloakAuthenticator.class).in(Scopes.SINGLETON);

                    // Configure HTTP client with reasonable defaults
                    httpClientBinder(binder)
                            .bindHttpClient("keycloak", ForKeycloakAuthenticator.class)
                            .withConfigDefaults(c -> {
                                c.setConnectTimeout(new Duration(10, SECONDS));
                                c.setRequestTimeout(new Duration(30, SECONDS));
                            });
                });
